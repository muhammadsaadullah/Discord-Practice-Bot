...
export async function ensureWalletExists(interaction: ChatInputCommandInteraction): Promise<WithId<userData> | void> {
    try {
        ...
        if (!pUser) {
            ...
            if (pUser.username !== user.username) updates.username = user.username;
            if (pUser.displayName !== user.displayName) updates.displayName = user.displayName;
            if (pUser.totalItemWorth === undefined) updates.totalItemWorth = 0;
            if (pUser.totalNetWorth === undefined) updates.totalNetWorth = pUser.balance + (pUser.totalItemWorth ?? 0);
            if (pUser.dailyStreak === undefined) updates.dailyStreak = 0;

            let TNWorth = pUser.balance + (pUser.totalItemWorth ?? 0)
            if (pUser.totalNetWorth !== TNWorth) updates.totalNetWorth = TNWorth;
