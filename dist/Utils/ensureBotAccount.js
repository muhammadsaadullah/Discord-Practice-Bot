...
function getBotCreationDate(botUserId) {
...
function convertToKarachiTime(date) {
...
export async function ensureBotAccount(client) {
    try {
        ...
        if (!botAcc) {
            ...
            if (!botAcc.createdAt || botAcc.createdAt.getTime() !== botCreationDateInKarachiTime.getTime())
                updates.createdAt = botCreationDateInKarachiTime; // Update the creation date in Karachi time
            if (botAcc.balance === 0 || botAcc.balance === undefined || botAcc.balance === null)
                updates.balance = 1000000;
            if (botAcc.totalNetWorth === undefined)
                updates.totalNetWorth = botAcc.balance + (botAcc.totalItemWorth ?? 0);
